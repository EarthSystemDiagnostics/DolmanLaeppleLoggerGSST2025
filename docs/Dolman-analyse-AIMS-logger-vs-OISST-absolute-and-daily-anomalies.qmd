...
## Time series figure for supplement
...
```{r}
logger_oisst_daily_10_anom %>% 
  group_by(site, site_run) %>% 
  summarise(var_abs = var(SST_daily),
            var_anom = var(SST_daily_anom)) %>% 
...
S_logger_OISST_mean <- AIMS_logger_spec_match_reg_mean %>% 
  ...
  facet_wrap(~type, scales = "free_y") +
  geom_vline(xintercept = c(1/10, 1/2, 2, 12, 182.5), lty = 3) +
  theme(panel.grid.major = element_blank()) +
  geom_text(data = ts_labs, aes(x=x, y=y, label = label),
            hjust = "left",
            inherit.aes = FALSE)
...
ratio_site_run_bw <- ratio_site_run_spec %>%
  ...
  #filter(freq > 12) %>%
   mutate(f_band_name = cut(freq, 
                      breaks = c(0.09, 1/2, 1/1.1, 1.1, 12, 
                                 182),
                      labels = c("Decadal to inter-annual",
                                 "Inter-annual to annual",
                                 "Annual",
                                 "Annual to monthly",
...
fig_depth_var_ratio <- ratio_site_run_bw %>% 
  ...
  facet_grid(f_band_name+f_band ~ ., labeller = labeller(.multi_line = FALSE), 
             scales = "fixed") 
...
ratio_site_run_spec %>% 
  ...
  #filter(freq > 12) %>% 
  mutate(f_band_name = cut(freq, 
                      breaks = c(0.09, 1/2, 2, 12, 
                                 182),
                      labels = c("Decadal to inter-annual",
                                 "Inter-annual to biannual",
                                 #"Annual",
                                 "Biannual to monthly",
                                 "Monthly to inter-daily")),
         f_band = cut(freq, 
...
```{r}
lag_cor <- logger_oisst_daily_10_anom_long %>% 
  filter(year.dec < 2024.708) %>% 
  group_by(type, SST_product, site, site_run) %>% 
  mutate(n = sum(is.na(SST_daily + logger_daily)==FALSE),
         start_year = min(year), 
  ...
  do({
    ...
    ccf1 <- ccf(dat$SST_daily, dat$logger_daily,  plot = FALSE)
...
```{r}
lag_cor %>% 
  ggplot(aes(x = lag, y = rho, group = site_run)) +
  geom_line() +
...
best_lags <- lag_cor %>% 
  group_by(type, site_run) %>% 
  summarise(best_lag = lag[which.max(rho)]) %>% 
  arrange(desc(abs(best_lag)))


best_lags %>% 
  ggplot(aes(x = best_lag)) +
  geom_histogram() +
  facet_wrap(~type)
...
AIMS_logger_oisst_coh_smooth %>% 
  group_by(type, SST_product, site_run, inc_2006_2008) %>%  
  filter(freq >= 1/10, freq <= 182) %>% 
  mutate(f_band_name = cut(freq, 
                      breaks = c(0.09, 1/1.1, 1.1, 12, 
                                 182),
                      labels = c("Decadal to inter-annual",
                                 "Annual",
                                 "Annual to monthly",
...
combined_fig_2 <- patchwork::wrap_plots(
  S_logger_OISST_mean +
    theme(
...
combined_fig_2_anom <- patchwork::wrap_plots(
  S_logger_OISST_anom +
    theme(
      axis.title.x.bottom = element_blank(),
      panel.grid = element_blank()
