...
AIMS_logger_spec_match <- logger_oisst_daily_10_anom %>% 
  ...
  do({
    ...
    # sp_tmp <- PaleoSpec::SpecACF(dat2$temperature,
    #                                 deltat = 1/365,
    #                              k = 1, nw = 1,
    #                              )  %>%
...
```{r}
#identical(AIMS_logger_spec_match_2$spec , AIMS_logger_spec_match_1$spec)

#table(AIMS_logger_spec_match_2$spec == AIMS_logger_spec_match_1$spec, useNA = #"always")
```


<!-- ```{r} -->
<!-- gg_spec(AIMS_logger_spec_match, colour = temperature_type, -->
<!--        group = paste(temperature_type, site_run)) + -->
...
# gg_spec(AIMS_logger_spec_match_reg, colour = sensor_type, type,
#        group = paste(sensor_type, type, site_run)) +
#   facet_wrap(~temperature_type)
...
ratio_site_run_spec <- AIMS_logger_spec_match_reg %>% 
  ...
  mutate(type = ifelse(grepl("anom", spec_id, fixed = TRUE),
                       "Daily anomalies", "Daily means")) %>% 
  group_by(type) %>% 
 #  mutate(lim.1 = qf(c(1 - pval / 2), dof, dof) * spec,
 #          lim.2 = qf(c(pval / 2), dof, dof) * spec 
 # ) %>% 
  as_spec_df() 
...
fig_depth_var_ratio <- ratio_site_run_bw %>% 
  ...
  facet_grid(f_band_name+f_band ~ type, labeller = labeller(.multi_line = FALSE), 
             scales = "fixed") 
