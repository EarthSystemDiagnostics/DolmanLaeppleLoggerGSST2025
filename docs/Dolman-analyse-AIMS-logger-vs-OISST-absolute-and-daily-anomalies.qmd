...
fig_depth_var_ratio <- ratio_site_run_bw %>% 
  ...
  facet_grid(f_band_name+paste0("Freq. band ", f_band) ~ .,
...
# ratio_site_run_bw %>% 
fig_depth_var_ratio_sq <- ratio_site_run_bw %>% 
  filter(type == "Daily anomalies") %>% 
  ggplot(aes(x = depth, y = spec)) +
  geom_point(alpha = 0.5) +
  geom_hline(yintercept = 1, lty = 2) +
  ...
  #facet_grid(f_band_name+paste0("Freq. band ", f_band) ~ .,
   #          labeller = labeller(.multi_line = TRUE), 
  #          scales = "fixed") 
  facet_wrap(~f_band_name+paste0("Freq. band ", f_band),
             labeller = labeller(.multi_line = TRUE), 
             scales = "fixed") 

fig_depth_var_ratio_sq
```
...
fig_coh_smooth_anom <- AIMS_logger_oisst_coh_smooth %>% 
  ...
  labs(colour = "Depth [m]", x = "Frequency [1/years]",
       y = "Squared coherency") +
  geom_vline(xintercept = c(1/10, 1/2, 2, #12,
                            182.5), lty = 3) +
 freq_axis
